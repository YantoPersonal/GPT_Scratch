"""
Bi-gram.py - Positional Encoding (1:00:21)
by: Ieuan Griffiths
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # New: Position Embedding: pos_emb grabs the embeddings for each value from 0 up to T-1 = 7 (1 to 8 basically).
        pos_emb = self.position_embedding_table(torch.arange(T, device=device)) # (T, N embd)
        # New: The input into the logits is a combination of the token embedding, and it's location.
        # Essentially, if the letter a shows up in the 5th or 6th position, it's now a different value due to pos_emb.
